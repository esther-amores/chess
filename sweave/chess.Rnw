...
Així doncs, podem veure que totes les altres mesures (TPR, TNR, PPV, F1) que hem calculat per saber com de bé s'estima les xarxes bayesianes, les del Augmented Naive Bayes són més elevades (>95\%) que les del Naive Bayes.
...
Observem, finalment, que el classificador Augmented Naive Bayes obté unes mètriques molt bones (>94\%). Podem veure la seva corresponent estructura generada amb la funció \texttt{graphviz.plot} a la Figura \ref{fig:augmented_nb}.
...
graphviz.plot(xarxa)
@
     \caption{Graf del classificador Augmented Naive Bayes amb l'aprenentatge sobre el conjunt d'entrenament i la validació sobre el de test}
     \label{fig:augmented_nb}
     \end{center}
\end{figure}


Al següent apartat ja podrem entrenar la nostra xarxa bayesiana final amb \textbf{totes} les dades fent servir un Augmented Naive Bayes, per després procedir a fer la validació.
...
xarxa.grain <- suppressWarnings(as.grain(xarxa.estimada))
@
     \caption{Graf del classificador Augmented Naive Bayes amb l'aprenentatge sobre el conjunt d'entrenament i la validació sobre el de test ((crec que aquest no cal posar-lo perquè és fa igual que el d'abans, osigui les primeres 4 línies de codi les podríem eliminar directament))}
     \label{fig:augmented_nb_final}
     \end{center}
\end{figure}
...
# plot(chss4)
@
     \includegraphics[width=0.5\textwidth]{fotos/kr-vs-kp2.png}
     \caption{Diagrama d'escacs. Torre i rei negres contra rei i peó blancs.}
     \label{fig:chss4}
     \end{center}
\end{figure}
...
a <- c(F, T, F, F, F, T, T, F, T, F, T, T, "g", F, "n", F, F, F, F, F, F, T, F, F, T, F, F, T, F, F, T, F, F, F, T, F, "n")
a <- A[-c(5,13,19,24,26,28,29,36)]

atributes <- colnames(dades)
