...
\affil[1,2,3]{Estudiant d'Estadística Aplicada -- Universitat Autònoma de Barcelona}
%\affil[2]{Applied Statistics Student -- Universitat Autònoma de Barcelona}
%\affil[3]{Applied Statistics Student -- Universitat Autònoma de Barcelona}
...
L'escac i mat és l'expressió emprada per a indicar que el rei que està en escac no el pot eludir, i per tant la partida és acabada \cite{escacimat}.
...
# plot(chss)
@
     \includegraphics[width=0.5\textwidth]{fotos/chess.png}
...
# plot(chss2)
@
     \includegraphics[width=0.5\textwidth]{fotos/kr-vs-kp1.png}
...
Cal esmentar, que la distribució que pren aquesta variable \textbf{target} està força equilibrada, és a dir: en 1669 casos (52\%) les blanques guanyen (valor ``win''), mentre que en 1527 casos (48\%) les blanques no guanyen (valor ``nowin'').
...
\begin{description}
    ...
    \item[bknwy]El rei negre no molesta en cap moviment de la torre negra, és a dir, no està en el camí de la torre negra.
    ...
    \item[bkspr]El rei negre pot defensar la torre negra. En la Figura \ref{fig:chss2} el rei negre ajuda a torre negre quan torre negre mou a a4.
    \item[bkxbq]El rei negre no està atacat de cap manera pel peó coronat.
    \item[bkxcr]El rei negre pot atacar la casella crítica b7.
    \item[bkxwp]El rei negre pot atacar el peó blanc.
    \item[blxwp]Les peces negres ataquen al peó blanc (torre negre només pot en direcció x=-1, és a dir, cap a f7).
    \item[bxqsq]Una o més peces negres (és a dir, o el rei o la torre) controlen el quadrat de la coronació a8.
    ...
    \item[dwipd]La distància del rei blanc al punt d'intersecció és gran (\textit{great}, g) o petita (\textit{low}, l). El punt d'intersecció és aquella casella que talla la torre. En el cas de la Figura \ref{fig:chss2}, per exemple, el punt d'intersecció és f4 perquè és el punt on ha d'arribar el rei blanc per aconseguir alliberar-se, ja que el rei blanc no pot moure's ni per la fila 4 ni per la columna G, i s'ha d'acostar a la torre negra per atacar-la.
    ...
    \item[katri]El rei negre controla el punt d'intersecció de les blanques (\textit{white}, w), negres (\textit{black}, b) o cap (\textit{none}, n).
    ...
    \item[skach]Es pot fer 1 o més escacs al rei blanc per haver sacrificat el peó.
    ...
    \item[spcop]Hi ha una oposició especial entre dos reis. És a dir, entenem que no és la oposició estàndard (com la de la variable \textbf{\texttt{dsopp}})
    ...
    \item[thrsk]Es fa escac a la descoberta (una peça que està interferint l'acció d'una segona, s'aparta del seu camí) fent un doble atac.
    \item[wkcti]El rei blanc no pot controlar el punt d'intersecció. En el cas de la Figura \ref{chss2}, el punt d'intersecció seria f4.
...
És a dir, l'\textbf{objectiu} és construir una Xarxa Bayesiana de manera que, donada unes posicions concretes on només tenim la torre i el rei negres i, el rei i el peó blancs (aquest últim està en la posició del tauler a7), es pugui estimar la probabilitat que les blanques puguin guanyar o no.

Com que aquesta Xarxa Bayesiana, es farà servir per classificar si poden guanyar o no les blanques, s'utilitzaran dos possibles classificadors bayesians: el \emph{Naive Bayes} i l'\emph{Augmented Naive Bayes}.

En definitiva, per poder assolir aquest objectiu: en primer lloc es farà un preprocessing de la base de dades, seguidament, es separarà la base de dades amb un conjunt d'entrenament i un de test. Amb el d'entrenament, es durà a terme el procediment de k-fold cross-validació per tal d'entrenar i validar tant el Naive Bayes com l'Augmented Naive Bayes (s'utilitzarà el paquet \texttt{bnlearn} per entrenar cadascuna de les xarxes). També, a partir de les mesures de comportament triarem aquell que sigui l'òptim. Per altra banda, amb la xarxa escollida i el conjunt test farem una segona validació per acabar de constatar si la xarxa escollida funciona bé. I finalment, a partir de tota la base de dades, s'aprendrà el classificador bayesià escollit, per a fer la predicció de la classe de nous casos i predigui la classe d'alguns casos que ens han semblat interessants (mitjançant el paquet \texttt{gRain}).
...
En primer lloc, es carrega la base de dades al directori on es treballa i s'assegura que no hi hagi cap valor missing amb la funció \texttt{anyNA}. A més, s'assigna totes les variables com a categòriques amb la funció \texttt{factor} i es mira un resum descriptiu de cadascuna de les variables, el qual podem veure a la Taula \ref{tab:freqs}.
...
Cal destacar que la variable \textbf{spcop} obté una distribució de les dades molt anòmala ja que només existeix un cas on aquesta variable prén el valor \texttt{TRUE}. D'aquesta manera, es tindrà en compte alhora de triar les variables que s'utilitzin per entrenar les xarxes.

A més a més, es pot veure que cap de les variables conté valors perduts, i per tant, és un fet destacable i que també s'ha de tenir en compte quan s'aprenen les xarxes.

Pel que fa al preprocessament de les dades, primerament s'ajusta un model de regressió logística amb la funció \texttt{glm} (veure Taula \ref{tab:glm}), ja que la variable classe del dataset és dicotòmica (0-blanques no poden guanyar, 1-blanques poden guanyar).
...
En aquest apartat i a partir del conjunt d'entrenament es durà a terme l'aprenentatge de l'estructura i dels paràmetres del classificador Naive Bayes així com la seva validació, mitjançant el procediment de k-fold cross-validació amb $k=10$.

És a dir, aquest conjunt d'entrenament es separarà en 10 parts proporcionals, que tindràn aproximadament el mateix nombre de casos. Llavors es començarà un procés iteratiu on es  repetirà 10 vegades el següent: s'aprendrà tant l'estructura com els paràmetres del classificador a partir del conjunt d'entrenament format per 9 trossos. Pel tros restant, es farà la validació, és a dir, per cada cas d'aquest s'entren els inputs al classificador entrenat i fem la predicció per l'output.
...
A continuació s'explica amb més detall en què consisteix l'aprenentatge de l'estructura i dels paràmetres així com de la seva validació que es desenvolupa a cada iteració. També es mostrarà quines són les mesures de comportament que s'obtenen.
...
Com ja s'ha esmentat, s'ha fet l'aprenentatge de l'estructura de la xarxa mitjançant el mètode heurístic de \textit{Search-and-score}. Aquest mètode utilitza una funció de puntuació i l'algoritme intenta trobar l'estructura que la maximitzi, per això farem servir la funció \texttt{hc} de la llibreria \texttt{bnlearn}. En aquest cas, el nostre dataset conté només dades completes, com bé hem dit abans.

Com volem que el classificador bayesià sigui un Naive Bayes, introduïrem una \textit{white list} perquè ens interessa que els arcs vagin des de la variable classe (\textbf{target}) fins a tota la resta de variables explicatives, i una \textit{black list} per evitar que les variables explicatives tinguin fletxes entre elles.
...
L'últim pas és fer que el classificador bayesià que hem aprés a cadascuna de les iteracions, predigui per a cada cas de cadascun dels folds destinats a la validació, la probabilitat que assigni si les peces blanques podran guanyar o no.
...
<<structure, results='hide', cache=TRUE>>=
...
Quan volem que el classificador bayesià sigui un Augmented Naive Bayes, introduïm una \textit{white list} perquè ens interessa que els arcs vagin des de la variable classe (\textbf{target}) fins a tota la resta de variables explicatives, però en aquest cas no afegim una \textit{black list} perquè no ens cal evitar que les variables explicatives tinguin fletxes entre elles.
...
De manera similar a l'aprenentatge de paràmetres del Naive Bayes, s'ha estimat els paràmetres, és a dir, les probabilitats de cada node condicionat al seus pares per a cada iteració.
...
Com ja s'ha explicat a la validació pel classificador del Naive Bayes, \ref{sec:validacio}, l'últim pas és fer que el classificador bayesià que hem aprés per cadascuna de les iteracions, predigui per cada cas de cadascun dels folds destinats a la validació, la probabilitat que assigni si les peces blanques podran guanyar o no.

D'aquesta manera, s'obté la matriu de confusió de cada iteració, així com les mesures de comportament calculades, com es pot visualitzar a continuació:


<<augmented_structure, results='hide', cache=TRUE>>=
atributes <- colnames(dades[-ncol(dades)])
wl <- data.frame(from = rep("target", length(atributes)), to = atributes)
...
Com que amb el procés de validació de cada xarxa, s'ha aconseguit una mostra de la mesura Accuracy, s'ha mirat si aquestes dues mostres segueixen una distribució de normalitat, a través del test de Shapiro Wilk. Aquest considera com a hipòtesi nul·la que la mostra en qüestió segueix la distribució normal.
...
S'ha vist que efectivament, els dos p-valors són majors a 0.05 (respectivament són \Sexpr{round(normalitat.naive, 3)} i \Sexpr{round(normalitat.augmented, 3)}), i per tant, no tenim evidències per rebutjar aquesta hipòtesi de normalitat a les dues mostres.

D'aquesta manera, s'ha desenvolupat un t-test de mostres aparellades, per veure si les mitjanes de les accuracies per cada fold del Naive Bayes i Augmented Naive Bayes es poden considerar diferents. Aquest test considera com a hipòtesi nul·la que les mitjanes de les dues mostres són iguals. No obstant, el p-valor ha resultat ser menor a 0.001 i, per tant, tenim evidències per rebutjar aquesta hipòtesi nul·la i considerar que les mitjanes de les accuracies són diferents.
...
Així doncs, podem veure que totes les altres mesures (\textit{True Positive Rate}, \textit{True Negative Rate}, \textit{Positive Predictive Value}, \textit{F1 score}) que hem calculat per saber com de bé s'estima les xarxes bayesianes, les del Augmented Naive Bayes són més elevades (>95\%) que les del Naive Bayes.


%%%%%%%%%%%%%%%
%%% 
%%% Subsubsection: Aprenentatge i validació pel conjunt test del classificador escollit 
%%%
%%%%%%%%%%%%%%%
\subsubsection{Aprenentatge i validació pel conjunt test del classificador escollit}
\label{sec:augmented_structure_final}

Finalment el classificador escollit és l'Augmented Naive Bayes i, en aquest cas, tornarem a fer l'aprenentatge de l'estructura, considerant una \textit{white list} que forçi tot els arcs de la variable classe (\textbf{target}) fins a la resta de variables, però sense una \textit{black list} per a que el model pugui crear arcs entre les variables explicatives. També farem l'aprenentage dels paràmetres i ambdós es faran a partir del conjunt d'entrenament. Un cop fet aquests aprenentatge, es fa la validació sobre el conjunt tests. Les mesures de comportament obtingudes es poden veure a la Taula \ref{tab:mesures_test}.
...
Al següent apartat ja podrem entrenar la nostra xarxa Bayesiana final amb \textbf{totes} les dades fent servir un Augmented Naive Bayes, per després procedir a fer les prediccions. 


%%%%%%%%%%%%%%%%
%%%
%%% Section: Aprenentage i predicció per a noves partides amb el classificador final
%%%
%%%%%%%%%%%%%%%%
\section{Aprenentage i predicció per a noves partides amb el classificador final}
...
Un cop s'ha après l'estructura i els paràmetres del classificador Augmented Naive Bayes a partir de tota la base de dades, ja estem preparats per poder simular noves partides d'escacs on es doni aquesta situació de final de partida on les peces blanques tenen el rei i un peó, mentre que les negres tenen el rei i una torre. No obstant, com es tracta d'un Augmented Naive Bayes, tenint molts nodes i molts arcs entre aquests (veure Figura \ref{fig:augmented_nb}), la distribució de probabilitat conjunta no obtindrà probabilitats per totes i cadascuna de les possibles combinacions que es poden donar amb aquestes variables. Per tant, pot ser que les probabilitats de predicció no es puguin calcular. En aquests casos que hem considerat rellevant classificar, aplicarem la xarxa Bayesiana amb Naive Bayes, ja que aquest té la propietat de predir per a qualsevol nou cas. Podem veure la corresponent estructura de l'Augmented Naive Bayes i la del Naive Bayes generades amb la funció \texttt{graphviz.plot} a la Figura \ref{fig:augmented_nb} i \ref{fig:nb}, respectivament.
...
Amb el Naive Bayes predirem dues propostes de partides d'escacs, que es poden veure representades a les Figures \ref{fig:chss3} i \ref{fig:chss4} i les seves corresponents prediccions estan a la Taula \ref{tab:chss3_pred}.
...
Amb l'Augmented Naive Bayes també predim dos finals de partides, i les seves corresponents prediccions estan a la Taula \ref{tab:chss4_pred}.
...
\begin{thebibliography}{99}
   ...
   \bibitem{final} Final (escacs) \textit{Finals contra rei i peó.} \url{https://ca.wikipedia.org/wiki/Final_(escacs)} [Online; visitat el 27 de març de 2021]
    ...
    \bibitem{escacimat} escac i mat! \url{https://www.enciclopedia.cat/ec-gec-0229671.xml} [Online; visitat el 8 d'abril de 2021]
\end{thebibliography}
...
<<chss3, results="hide">>=
# https://www.apronus.com/chess/diagram/editor/
# chss3 <- Chess$new("8/P4r2/8/K1k5/8/8/8/8 w - - 0 1")
# plot(chss3)
@
     \includegraphics[width=0.5\textwidth]{fotos/kr-vs-kp3.png}
     \caption{Diagrama d'escacs. Torre i rei negres contra rei i peó blancs. Jugada nova 1.}
     \label{fig:chss3}
     \end{center}
\end{figure}
...
<<chss4, results="hide">>=
# https://www.apronus.com/chess/diagram/editor/
# chss4 <- Chess$new("8/Pk6/8/2r5/8/K7/8/8 w - - 0 1")
# plot(chss4)
@
     \includegraphics[width=0.5\textwidth]{fotos/kr-vs-kp4.png}
     \caption{Diagrama d'escacs. Torre i rei negres contra rei i peó blancs. Jugada nova 2.}
     \label{fig:chss4}
     \end{center}
\end{figure}
...
\begin{figure}[ht!]
     \begin{center}
<<augmented_naive_bayes2, results="hide">>=
graphviz.plot(xarxa.augmented)
@
     \caption{Graf del classificador Augmented Naive Bayes final}
     \label{fig:augmented_nb}
     \end{center}
\end{figure}
...
\begin{figure}[ht!]
     \begin{center}
<<naive_bayes2, results="hide">>=
graphviz.plot(xarxa.naive)
@
     \caption{Graf del classificador Naive Bayes final}
     \label{fig:nb}
     \end{center}
\end{figure}
